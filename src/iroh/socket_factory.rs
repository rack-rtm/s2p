...
pub trait SocketFactory: Send + Sync + std::fmt::Debug {
    fn create_tcp_connection(&self, addr: SocketAddr) -> Pin<Box<dyn Future<Output = Result<TcpStream, io::Error>> + Send + '_>>;
    
    fn create_udp_socket(&self, bind_addr: &str) -> Pin<Box<dyn Future<Output = Result<UdpSocket, io::Error>> + Send + '_>>;
}
